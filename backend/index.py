...
class AudioVideoProcessor:
    ...
    async  def extract_speech_segments_and_cluster(self, websocket,client_id, input_audio, output_dir, n_clusters=2,language='en'):
        ...
        response={"status": "Reading & Denoising Audio", "progress": 40, "time_left": 0,'reqdatajson':{}}
        ...
        response={"status": "Removing Silence", "progress": 45, "time_left": 0,'reqdatajson':{}}
        await websocket.send(json.dumps({"clientId": client_id,'response':response}))
        ...
        for i, (start, end) in enumerate(merged_segments):
            response={"status": f"Processing Segment {i+1}", "progress": 50 + (i * 5), "time_left": 0,'reqdatajson':{}}
            await websocket.send(json.dumps({"clientId": client_id,'response':response}))
        ...
        response={"status": "Clustering Speakers", "progress": 60, "time_left": 0,'reqdatajson':{}}
        await websocket.send(json.dumps({"clientId": client_id,'response':response}))
    ...
    async def add_subtitles_to_video(self,websocket,client_id, video_path, subtitles, final_output_path):
        ...
        while cap.isOpened():
            ...
            if not ret:
                break
            response={"status": f"Adding Subtitle {frame_no}", "progress": 80 + int((frame_no / fps) * 20), "time_left": 0,'reqdatajson':{}}
            await websocket.send(json.dumps({"clientId": client_id,'response':response}))
    ...
    async def add_subtitles_to_video_translate(self,websocket,client_id, video_path, subtitles, final_output_path,src,lang):
        cap = cv2.VideoCapture(video_path)
        fps = int(cap.get(cv2.CAP_PROP_FPS))
        width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
...
async def handler(websocket):
    ...
    predicted_segments, _ = await processor.extract_speech_segments_and_cluster(websocket,"1",audio_output_path, "output_segments", n_clusters=3,language=input_lang)
    await processor.add_subtitles_to_video_translate(websocket,"1",video_path, predicted_segments, final_output_path,input_lang,output_lang)

async def main():
    print("Python WebSocket Server Started at ws://localhost:8765")
    
    # ✅ Wrap handler using functools.partial to remove 'path' argument
    server = websockets.serve(functools.partial(handler), "localhost", 8765)
