...
class AudioVideoProcessor:
    ...
    def extract_speech_segments_and_cluster(self, input_audio, output_dir, n_clusters=2,language='en'):
    ...
    def add_subtitles_to_video(self,video_path, subtitles, final_output_path):
    ...
    # Ensure the model and inputs are on the same device
        device = self.device  # Define a device variable (e.g., "cuda" or "cpu")

        if src_lang == "ar" and tgt_lang == "en":
            self.model_ar_en.to(device)  # Move model to the correct device
            inputs = self.tokenizer_ar_en(text, return_tensors="pt", padding=True, truncation=True).to(device)
            ...
            self.model_en_ar.to(device)
            inputs = self.tokenizer_en_ar(text, return_tensors="pt", padding=True, truncation=True).to(device)
            ...
            self.model_ru_en.to(device)
            inputs = self.tokenizer_ru_en(text, return_tensors="pt", padding=True, truncation=True).to(device)
            ...
            self.model_en_ru.to(device)
            inputs = self.tokenizer_en_ru(text, return_tensors="pt", padding=True, truncation=True).to(device)
            ...
            self.model_ar_ru.to(device)
            inputs = self.tokenizer_ar_ru(text, return_tensors="pt", padding=True, truncation=True).to(device)
            ...
            self.model_ru_ar.to(device)
            inputs = self.tokenizer_ru_ar(text, return_tensors="pt", padding=True, truncation=True).to(device)
    ...
    def add_subtitles_to_video_translate(self, video_path, subtitles, final_output_path,src,lang):
        cap = cv2.VideoCapture(video_path)
        fps = int(cap.get(cv2.CAP_PROP_FPS))
        width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
...
processor.add_subtitles_to_video_translate(video_path, predicted_segments, final_output_path,input_lang,output_lang)
